...
writeBlocks ::
  ...
  FilePath -> ChunkNo -> NonEmpty blk -> IO (NonEmpty FilePath)
...
writeOneBlockOnly ::
  forall blk m h.
  (Serialise blk, Monad m) => HasFS m h -> Handle h -> blk -> m (Word64, CRC)
...
prop_iterationCountIsCorrect :: Property
prop_iterationCountIsCorrect =
...
tests =
  testGroup
    "iteration"
    [testProperty "Basic iteration over headers is correct" prop_iterationCountIsCorrect]
